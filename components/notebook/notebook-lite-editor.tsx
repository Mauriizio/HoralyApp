...
import { createContext, forwardRef, useCallback, useContext, useEffect, useImperativeHandle, useState, type ComponentProps, type MutableRefObject, type ReactElement } from "react"
import { $getNodeByKey, $getRoot, $getSelection, $insertNodes, $isParagraphNode, $isRangeSelection, COMMAND_PRIORITY_EDITOR, createCommand, DecoratorNode, FORMAT_TEXT_COMMAND, type EditorConfig, type LexicalCommand, type NodeKey, type SerializedLexicalNode, type Spread } from "lexical"
...
function Toolbar({ onPhoto }: { onPhoto: (file: File) => void }) {
  const [editor] = useLexicalComposerContext()
  ...
  return <div className="sticky top-0 z-10 flex max-w-full items-center gap-1 overflow-x-auto rounded-lg border bg-background/95 p-1.5 backdrop-blur" aria-label="Formato del apunte">
    ...
    <details className="relative"><summary className="flex min-h-11 cursor-pointer list-none items-center gap-2 rounded-md px-3 text-sm hover:bg-muted"><ImagePlus className="size-4" />Foto</summary><div className="absolute left-0 top-12 z-30 min-w-52 space-y-1 rounded-lg border bg-popover p-2 shadow-lg">
      <label className="flex min-h-11 cursor-pointer items-center gap-2 rounded px-3 text-sm hover:bg-muted"><Camera className="size-4" />Tomar foto<input data-testid="camera-input" className="sr-only" type="file" accept="image/*" capture="environment" onChange={(event) => { const file = event.target.files?.[0]; if (file) onPhoto(file); event.currentTarget.value = "" }} /></label>
      <label className="flex min-h-11 cursor-pointer items-center gap-2 rounded px-3 text-sm hover:bg-muted"><ImagePlus className="size-4" />Elegir de galería<input data-testid="gallery-input" className="sr-only" type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => { const file = event.target.files?.[0]; if (file) onPhoto(file); event.currentTarget.value = "" }} /></label>
    </div></details>
  </div>
}

function Commands({ api }: { api: MutableRefObject<NotebookLiteEditorHandle | null> }) {
  const [editor] = useLexicalComposerContext()
