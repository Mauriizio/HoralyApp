...
import { calculateRequiredGrade, calculateWeightedAverage, detectOverdueReminders, detectSubjectsAtRisk, determineNextClass, estimateWeeklyLoad, suggestBasicStudyBlock } from "../domain/academic-engine/index.ts"
...
test("motor académico detecta riesgo, vencidos, próxima clase y carga", () => {
  ...
  assert.equal(detectSubjectsAtRisk(data).length, 1)
