...
test("reminder assessment conserva título, materia, fecha, ticker y acción", () => {
  ...
  const assessment = messages.find((item) => item.key === "reminder:exam")!
  assert.equal(assessment.kind, "assessment")
  assert.match(assessment.message, /Taller Grupal N°1.*ÁLGEBRA Y TRIGONOMETRÍA.*8 de septiembre.*05:00/)
...
test("copy de evaluación diferencia hoy y mañana sin perder hora, título ni materia", () => {
  ...
  assert.equal(today.message, "Evaluación hoy a las 11:30: Control 1 de Física.")
  assert.equal(tomorrow.message, "Evaluación mañana: Taller Grupal N°1 de Álgebra a las 05:00.")
})
...
test("modelo puro del ticker mantiene 34 px/s, loop y umbral de drag", () => {
  assert.equal(ACADEMIC_TICKER_SPEED, 34)
  assert.equal(advanceAcademicTicker(10, 1_000, 100), 44)
  assert.equal(advanceAcademicTicker(90, 1_000, 100), 24)
...
test("shell, ticker, companion y formulario declaran contratos UX V2", async () => {
  ...
  assert.match(ticker, /Pausar noticias académicas/)
  assert.match(ticker, /Reanudar noticias académicas/)
  assert.match(ticker, /Anterior/)
  assert.match(ticker, /Siguiente/)
