...
export function createSchedulePdfRenderer(): SchedulePdfRenderer {
  ...
  return { async render(model) {
    ...
    doc.setFont("helvetica", "bold"); doc.setFontSize(18); doc.text("Horarily", margin, 34); doc.text(model.title, margin, 58)
    ...
    const tableTop = 132, timeWidth = 88, colWidth = (780 - timeWidth) / model.days.length
    const footerY = 580, legendHeight = model.legend.length ? 34 : 8
    const availableHeight = footerY - tableTop - legendHeight
    const rowHeight = Math.max(30, Math.min(52, availableHeight / Math.max(1, model.modules.length)))
    ...
    doc.setFont("helvetica", "normal"); doc.setFontSize(8); doc.text("Creado con Horarily · horaly-app.vercel.app", margin, 580)
