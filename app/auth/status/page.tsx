...
export default async function AuthStatusPage({ searchParams }: { searchParams: Promise<{ code?: string }> }) {
  ...
  return <main className="min-h-screen grid place-items-center bg-muted/30 p-4"><Card className="w-full max-w-md"><CardHeader className="relative pr-12"><Button variant="ghost" size="icon" className="absolute right-3 top-3" aria-label="Volver a Horaly" asChild><Link href="/"><X className="h-4 w-4" aria-hidden /></Link></Button><CardTitle>{item.title}</CardTitle><CardDescription>{item.description}</CardDescription></CardHeader><CardContent className="grid gap-2"><Button asChild><Link href={safeCode === "password-recovery-ready" ? "/auth/update-password" : "/auth/login"}>{safeCode === "password-recovery-ready" ? "Crear nueva contraseña" : "Ir a iniciar sesión"}</Link></Button><Button variant="outline" asChild><Link href="/auth/reset-password">Solicitar nuevo enlace</Link></Button><Button variant="ghost" asChild><Link href="/auth/register">Crear cuenta</Link></Button><Button variant="ghost" asChild><Link href="/">Volver a Horaly</Link></Button></CardContent></Card></main>
}
