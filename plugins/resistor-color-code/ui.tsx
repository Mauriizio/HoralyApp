...
export default function ResistorColorCodeTool({ copyText }: ToolPluginProps) {
  ...
  return <div className="space-y-5" aria-label="Calculadora de código de colores de resistencias">
    ...
    <div className="flex flex-wrap gap-2"><Button type="button" onClick={async () => { setCopied(await copyText(summary)) }}>{copied ? "Copiado" : "Copiar resultado"}</Button><Button type="button" variant="outline" onClick={() => { setBandCount(4); setColors(["marrón", "negro", "rojo", "oro", "marrón", "marrón"]); setValue("4700"); setCopied(false) }}>Reiniciar</Button><Button type="button" variant="ghost" onClick={() => { setBandCount(5); setColors(["marrón", "negro", "negro", "naranja", "marrón", "marrón"]); setValue("100") }}>Ejemplo 100 kΩ</Button></div>
  </div>
}
